    ...
    <div class="mt-6 grid md:grid-cols-12 md:gap-x-8">
      ...
      <div class="grid grid-cols-1 gap-y-10 gap-x-6 mt-4 sm:grid-cols-2 md:mt-0 md:col-span-9 lg:grid-cols-3 xl:gap-x-8">
        ...
        <template v-else>
          ...
          <template v-else>
            <a :href="projet.link" v-for="projet in allProjets.sort((a, b) => b.id - a.id)" target="_blank" class="group relative shadow-lg rounded-lg md:p-0 md:shadow-transparent md:rounded-none hover:opacity-80 transition ease-in duration-200">
